var QR = (function() {
  ...
  var api = {
    ...
    draw: function(str, canvas, cavW, cavH, ecc, src) {
      ...
      size = roundedSize;
      // ctx.rect(0, 0, cavW, cavW);
      // ctx.clearRect(0, 0, cavW, cavW);

      ctx.setFillStyle('#fff');
      ctx.fillRect(0, 0, cavW, cavW);

      // const grd = ctx.createLinearGradient(0, 0, cavW, cavW)
      // grd.addColorStop(0, '#94decf')
      // grd.addColorStop(1, '#5e93d7')
      // ctx.setFillStyle(grd)

      ctx.setStrokeStyle('#27A9DC')
      ctx.setLineWidth(10)
      ctx.strokeRect(0, 0, cavW, cavW)

      ctx.setFillStyle('#000');
      for (var i = 0; i < width; i++) {
        for (var j = 0; j < width; j++) {
          if (frame[j * width + i]) {
            ctx.fillRect(px * (2 + i) + offset, px * (2 + j) + offset, px, px);
          }
        }
      }
      if (src) {
